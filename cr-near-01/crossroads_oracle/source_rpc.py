"""Source-chain (Sepolia) reader with multi-RPC quorum.

Request-driven counterpart to the Model A push loop: given a block number, query
every configured source RPC, rebuild the canonical RLP header from each, require
keccak(rlpHeader)==block.hash per RPC, and only accept a block hash that a quorum
of independent RPCs recompute identically. The winning RLP header + vote record
feed a signed report. One lagging RPC cannot stall `latest-confirmed` as long as
quorum still holds.
"""
...
class SourceRpcClient:
    def __init__(self, config: SourceRpcConfig, rpc_call: RpcCall | None = None) -> None:
        if config.quorum <= 0:
    ...
    def validate_sources(self) -> tuple[SourceEndpoint, ...]:
        ...
        for index, url in enumerate(self.config.urls):
            ...
            if chain_id != self.config.chain_id:
                print(
                    f"[source-rpc] rejected {url}: chainId {chain_id} != {self.config.chain_id}",
                    flush=True,
                )
                continue
        ...
        if len(valid) < self.config.quorum:
            raise SourceUnavailableError(
                f"only {len(valid)} source RPCs matched chainId; quorum is {self.config.quorum}"
            )
    ...
    def _fetch_status(self) -> SourceStatus:
        ...
        if self.config.require_finalized:
            finalized_results = self._call_sources("eth_getBlockByNumber", ["finalized", False])
    ...
    def _fetch_confirmed_header(self, block_number: int, status: SourceStatus) -> ConfirmedHeaderResult:
        ...
        if self.config.require_finalized:
            ...
            if finalized_block_number is None:
                raise SourceUnavailableError("finalized quorum is unavailable")
            finalized_count = sum(
                1 for finalized in status.finalized_numbers.values() if finalized >= block_number
            )
