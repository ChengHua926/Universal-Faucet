...
from .header_report import build_signed_header_report
    ...
    def _refresh_chain_signer_state(self) -> None:
        ...
        if chain_signer.lower() != self.signer_address.lower():
            raise SignerMismatchError(
                f"signer_mismatch: chain has {chain_signer}, local signer is "
                f"{self.signer_address}"
            )
            ...
            try:
                ...
                if parsed.path == "/v1/header":
                    ...
                    if not block_values:
                        self._send_json(
                            400,
                            {"ok": False, "error": "missing block_number"},
                        )
                        return
                ...
                self._send_json(
                    exc.status_code,
                    {"ok": False, "error": exc.error_code, "message": exc.message},
                )
            except SignerMismatchError as exc:
                self._send_json(
                    503,
                    {"ok": False, "error": "signer_mismatch", "message": str(exc)},
                )
