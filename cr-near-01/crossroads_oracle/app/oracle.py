...
class OracleConfig:
    ...
    def from_env(cls) -> "OracleConfig":
        ...
        source_confirmations = _int_env(
            "SOURCE_CONFIRMATIONS", DEFAULT_SOURCE_CONFIRMATIONS
        )
        ...
        ttl_seconds = _int_env(
            "HEADER_REPORT_TTL_SECONDS", DEFAULT_HEADER_REPORT_TTL_SECONDS
        )
        if ttl_seconds <= 0:
        ...
        return cls(
            ...
            header_report_cache_size=_int_env(
                "HEADER_REPORT_CACHE_SIZE", DEFAULT_HEADER_REPORT_CACHE_SIZE
            ),
            header_report_cache_refresh_seconds=_int_env(
                "HEADER_REPORT_CACHE_REFRESH_SECONDS",
                DEFAULT_HEADER_REPORT_CACHE_REFRESH_SECONDS,
            ),
            http_host=os.environ.get("HTTP_HOST", DEFAULT_HTTP_HOST),
            http_port=_int_env("HTTP_PORT", DEFAULT_HTTP_PORT),
            http_rate_limit_per_minute=_int_env(
                "HTTP_RATE_LIMIT_PER_MINUTE", DEFAULT_HTTP_RATE_LIMIT_PER_MINUTE
            ),
            ...
            signing_key_path=os.environ.get(
                "ORACLE_SIGNING_KEY_PATH", DEFAULT_SIGNING_KEY_PATH
            ),
        )
...
def register_header_signer(
    config: OracleConfig, rofl: RoflAppdClient
) -> HeaderSignerStartup:
    if config.mode != "rofl":
    ...
    if normalize_rofl_app_id_bytes(app_id) != normalize_rofl_app_id_bytes(
        contract_rofl_app_id
    ):
        raise RuntimeError(
            "ROFL app id reported by appd does not match the oracle contract"
        )
    ...
    print(
        f"[signer] registration commitment for epoch {next_epoch}: {commitment}",
        flush=True,
    )
...
def main() -> None:
    ...
    print(
        f"[source-rpc] {len(valid_sources)} RPCs are usable; quorum is "
        f"{config.source_rpc_quorum}",
        flush=True,
    )
    ...
    service = HeaderReportService(
        ...
        cache=ReportCache(
            config.header_report_cache_size,
            config.header_report_cache_refresh_seconds,
        ),
    )
    run_http_server(
        config.http_host,
...
def _load_header_signer(config: OracleConfig, rofl: RoflAppdClient) -> Any:
    if config.allow_legacy_file_signer:
        print(
            "[signer] ALLOW_LEGACY_FILE_SIGNER=1; loading legacy file signer",
            flush=True,
        )
    ...
    print(
        f"[signer] using ROFL KMS key id {config.rofl_kms_key_id}",
        flush=True,
    )
