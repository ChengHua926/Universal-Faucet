"""ROFL KMS-derived header signer.

Asks appd for a deterministic secp256k1 key bound to THIS app (key_id) and
derives its Ethereum address. The key never touches disk: it lives only in
memory inside the TEE for the life of the process. Because appd re-derives the
same key for the same (app, key_id), the on-chain `headerSigner` registration
stays valid across restarts/updates — that determinism is the linchpin of
Model B's "register once" design.
"""
...
def _extract_private_key(response: Any) -> str:
    ...
    if isinstance(response, dict):
        for key in ("private_key", "privateKey", "key", "key_hex", "secret", "data"):
